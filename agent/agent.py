...
4. **内存深度诊断**：对目标应用执行 heap dump 分析（memory_diagnosis），找出内存占用最大的类，排查内存泄漏，如果hprof 文件过大，无法在此完成类级别的精确解析，则在建议中增加手动获取hprof文件的命令
...
async def stream_agent(message: str, chat_history: list = None):
    """
    流式调用 Agent — 实时 yield SSE 事件。
    事件类型:
      - thinking:   LLM 推理过程（决定调用工具前的思考，或给出最终答案前的总结）
      - tool_start: 工具开始执行（含工具名和输入参数）
      - tool_end:   工具执行完毕（含结果摘要）
      - tool_error: 工具执行出错
      - output:     最终 LLM 回复
      - done:       流结束
    """
